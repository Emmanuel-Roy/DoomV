...
5. `run()` starts a detached CPU thread. The original thread polls SDL input and composites what a display thread (whole guest frames) and a dashboard thread (the register panels) hand it. The CPU performs bursts of up to 200,000 `step()` calls before publishing a snapshot, committing queued input at instruction counts that are multiples of 4,096, so input timing depends on the instruction count and not on where a burst happened to be. `run()` also publishes one snapshot before the CPU thread starts, so the first frame already has the guest's display geometry.
